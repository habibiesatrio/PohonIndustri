...
const Dashboard = () => {
        ...
        return ( <
                ...
                <
                div className = "bg-red-600 p-2.5 rounded-xl shadow-lg shadow-red-100" > < LayoutDashboard className = "text-white w-6 h-6" / > < /div> <
                div >
                <
                ...
                <
                NavItem active = { activeDashTab === 'hilirisasi' }
                onClick = {
                    () => setActiveDashTab('hilirisasi')
                }
                icon = { < Layers size = { 18 }
                    />} label="Hilirisasi Nasional" / >
                    <
                    NavItem active = { activeDashTab === 'paten' }
                    onClick = {
                        () => setActiveDashTab('paten')
                    }
                    icon = { < Microscope size = { 18 }
                        />} label="Analitik Paten" / >
                        <
                        NavItem active = { activeDashTab === 'sektoral' }
                        onClick = {
                            () => setActiveDashTab('sektoral')
                        }
                        icon = { < Landmark size = { 18 }
                            />} label="Data Sektoral" / >
                            <
                            NavItem active = { activeDashTab === 'performance' }
                            onClick = {
                                () => setActiveDashTab('performance')
                            }
                            icon = { < Activity size = { 18 }
                                />} label="Performa Ekspor" / >
                                <
                                NavItem active = { activeDashTab === 'gis' }
                                onClick = {
                                    () => setActiveDashTab('gis')
                                }
                                icon = { < Map size = { 18 }
                                    />} label="Pemetaan GIS" / >
                                    <
                                    ...
                                    <
                                    div className = "w-10 h-10 rounded-full bg-red-600 flex items-center justify-center text-white font-black" > N < /div> <
                                    div >
                                    <
                                    p className = "text-xs font-black text-slate-800 uppercase" > Nugroho A.S < /p> <
                                    p className = "text-[9px] font-bold text-slate-400 uppercase" > Kepala Riset < /p> < /
                                    div > <
                                    /div> <
                                    button onClick = { handleLogout }
                                    className = "w-full flex items-center gap-3 p-3 text-slate-400 hover:text-red-600 font-bold text-xs uppercase transition-colors" >
                                    <
                                    LogOut size = { 18 }
                                    /> Logout Portal < /
                                    button > <
                                    ...
                                    p className = "text-slate-400 text-sm font-medium uppercase tracking-widest" > Update Data: 08 Jan 2025,
                                    14: 04 WIB < /p> < /
                                    div > <
                                    div className = "flex gap-4" >
                                    <
                                    button className = "bg-white border border-slate-200 p-2.5 rounded-xl text-slate-500 hover:bg-slate-50 shadow-sm transition-all" > < Bell size = { 20 }
                                    /></button >
                                    <
                                    button className = "bg-white border border-slate-200 p-2.5 rounded-xl text-slate-500 hover:bg-slate-50 shadow-sm transition-all" > < Settings size = { 20 }
                                    /></button >
                                    <
                                    ...
                                    { /* Dynamic Content Based on Tabs */ } {
                                        activeDashTab === 'hilirisasi' && ( <
                                            ...
                                            <
                                            MetricCard label = "Total Komoditas Strategis"
                                            val = "95"
                                            sub = "Sesuai Roadmap 2045"
                                            icon = { < Database className = "text-red-600" / > }
                                            /> <
                                            MetricCard label = "Maturitas Rata-rata"
                                            val = "TRL 7.2"
                                            sub = "Trend Meningkat"
                                            icon = { < Target className = "text-indigo-600" / > }
                                            /> <
                                            MetricCard label = "Nilai Tambah Agregat"
                                            val = "35.4x"
                                            sub = "Target: 50x"
                                            icon = { < TrendingUp className = "text-emerald-600" / > }
                                            /> <
                                            MetricCard label = "Status Swasembada"
                                            val = "64%"
                                            sub = "Proyeksi 2026"
                                            ...
                                            <
                                            Layers size = { 18 }
                                            className = "text-red-600" / > Detail Kesiapan Teknologi Hilir(TRL / MRL / CRL) <
                                            /h3> <
                                            ...
                                            cy = "50%"
                                            outerRadius = "80%"
                                            data = { hsDatabase[0].patentDistribution } >
                                            <
...

                                    {
                                        activeDashTab === 'paten' && ( <
                                            ...
                                            <
                                            Award size = { 18 }
                                            /> Inovasi Paling Berdampak (Top Patents) < /
                                            h3 > <
...

                                    {
                                        activeDashTab === 'sektoral' && ( <
                                            ...
                                            paddingAngle = { 5 }
                                            dataKey = "addedValue" > {
                                                sectoralData.map((e, i) => < Cell key = { i }
                                    ...
                                    const Badge = ({ label, val, color }) => ( <
                                        div className = { `px-2 py-1 rounded text-[10px] font-black ${color}` } > { label }: { val } <
                                        /div>
                                    );

                                    const NavItem = ({ active, onClick, icon, label }) => ( <
                                        button onClick = { onClick }
                                        className = { `w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-[11px] font-black uppercase transition-all ${active ? 'bg-red-600 text-white shadow-lg shadow-red-100' : 'text-slate-500 hover:bg-slate-50'}` } > { icon } { label } <
                                        /button>
                                    );
                                    ...
                                    const MetricCard = ({ label, val, sub, icon }) => ( <
                                        ...
                                        <
                                        div className = "w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-red-50 transition-colors" > { icon } <
                                        /div> <
                                        div >
                                        <
