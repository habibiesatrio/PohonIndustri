...
import { 
  ...
  AreaChart, Area, PieChart, Pie, RadarChart, PolarGrid, PolarAngleAxis, Radar, LineChart, Line
} from 'recharts';
import { 
  Search, Globe, ShieldCheck, ArrowRight, Zap, Calculator, AlertTriangle, 
  ChevronRight, Info, Database, Layers, Lightbulb, TrendingUp, Download, 
  RefreshCw, Landmark, TreeDeciduous, Factory, Microscope, LineChart as LucideLineChart,
  LogOut, LayoutDashboard, User, Bell, Settings, PieChart as LucidePieChart,
  BookOpen, Target, Activity, Award
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#fcfdfe] font-sans text-slate-900 flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-1">Pusat Kendali Hilirisasi</h2>
...
const NavItem = ({ active, onClick, icon, label }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-[11px] font-black uppercase transition-all ${active ? 'bg-red-600 text-white shadow-lg shadow-red-100' : 'text-slate-500 hover:bg-slate-50'}`}
  >
    {icon} {label}
  </button>
);
