            ...
            <aside className="w-full md:w-72 bg-white border-r border-slate-100 flex flex-col p-6 sticky top-0 h-screen z-50">
                ...
                <div className="mt-auto pt-6 border-t border-slate-50">
                     <div onClick={() => navigate('/profile')} className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl mb-4 cursor-pointer hover:bg-slate-100 transition-colors">
                        <div className="w-10 h-10 rounded-full bg-red-600 flex items-center justify-center text-white font-black">{(user?.nama || user?.email)?.charAt(0).toUpperCase() || 'U'}</div>
                        <div>
                        ...
                        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => setFlowFilter('All')} className={`px-4 py-2 text-xs font-bold rounded-lg ${flowFilter === 'All' ? 'bg-red-600 text-white' : 'bg-slate-100 text-slate-600'}`}>All</button>
                                    <button onClick={() => setFlowFilter('Export')} className={`px-4 py-2 text-xs font-bold rounded-lg ${flowFilter === 'Export' ? 'bg-red-600 text-white' : 'bg-slate-100 text-slate-600'}`}>Export</button>
                                    <button onClick={() => setFlowFilter('Import')} className={`px-4 py-2 text-xs font-bold rounded-lg ${flowFilter === 'Import' ? 'bg-red-600 text-white' : 'bg-slate-100 text-slate-600'}`}>Import</button>
                                </div>
                            </div>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {loading ? (
                                            ...
                                            <tr key={item.id} className="hover:bg-slate-50">
                                                ...
                                                <td className="p-3 text-center"><span className={`px-2 py-1 rounded text-xs font-black ${item.trl >= 8 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-50 text-red-600'}`}>{item.trl}</span></td>
                                                <td className="p-3 text-center"><span className={`px-2 py-1 rounded text-xs font-black ${item.mrl >= 8 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-50 text-red-600'}`}>{item.mrl}</span></td>
                                                <td className="p-3 text-center"><span className={`px-2 py-1 rounded text-xs font-black ${item.crl >= 8 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-50 text-red-600'}`}>{item.crl}</span></td>
                                                <td className="p-3"><span className={`flex items-center gap-1.5 text-xs font-bold ${item.drive === 'High Demand' ? 'text-red-600' : 'text-slate-500'}`}><Zap size={14} />{item.drive}</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
